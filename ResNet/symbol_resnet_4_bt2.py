...
def tucker_fc(input_layer,tag):
    W1 = mx.symbol.Variable(name='W1'+tag, shape=(I1 * J1 * Rank, 1), init=mx.init.Normal(stddev))
    W2 = mx.symbol.Variable(name='W2'+tag, shape=(1, I2 * J2 * Rank), init=mx.init.Normal(stddev))
    W3 = mx.symbol.Variable(name='W3'+tag, shape=(Rank * I3 * J3, 1), init=mx.init.Normal(stddev))
    W4 = mx.symbol.Variable(name='W4'+tag, shape=(1, Rank * I4 * J4), init=mx.init.Normal(stddev))
    W_core = mx.symbol.Variable(name='W_core'+tag, shape=(Rank ** 2, Rank ** 2), init=mx.init.Normal(stddev))
...
def resnet(units, num_stage, filter_list, num_class, data_type, bottle_neck=True, bn_mom=0.9, workspace=512,
    ...
    # fc1 = tucker_fc(flat)
    fc1 = tucker_fc(flat,'a')+tucker_fc(flat,'b')+tucker_fc(flat,'c')+tucker_fc(flat,'d')
