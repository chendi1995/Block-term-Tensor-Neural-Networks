...
import mxnet as mx
from symbol_resnet_TT8 import resnet
...
def main():
    ...
    if not os.path.exists("/hdd/chendi/model/"):
        os.mkdir("/hdd/chendi/model/")
    model_prefix = "/hdd/chendi/model/resnet-{}-{}-{}".format(args.data_type, args.depth, kv.rank)
    ...
    model.fit(train_data = train,
              ...
              eval_metric=['acc', 'ce'] if args.data_type == 'cifar10' else ['acc', mx.metric.create('top_k_accuracy', top_k=5)],
              kvstore=kv,
              arg_params=arg_params,
              aux_params=aux_params,
              num_epoch=200 if args.data_type == "cifar10" else 120,
              begin_epoch=begin_epoch,
              optimizer='nag',
              optimizer_params={'learning_rate': args.lr,'momentum':args.mom,'wd':args.wd, 'lr_scheduler':multi_factor_scheduler(begin_epoch, epoch_size, step=[120, 160], factor=0.1)
                             if args.data_type=='cifar10' else
                             multi_factor_scheduler(begin_epoch, epoch_size, step=[30, 60, 90], factor=0.1)},  # use fixed learning rate
