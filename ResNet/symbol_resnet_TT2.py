...
def tucker_fc(input_layer):
    W1 = mx.symbol.Variable('W1', shape=(I1 * J1, Rank), init=mx.init.Normal(stddev))
    W2 = mx.symbol.Variable('W2', shape=(Rank, I2 * J2 * Rank), init=mx.init.Normal(stddev))
    W3 = mx.symbol.Variable('W3', shape=(Rank * I3 * J3, Rank), init=mx.init.Normal(stddev))
    W4 = mx.symbol.Variable('W4', shape=(Rank, I4 * J4), init=mx.init.Normal(stddev))
    ...
    s1_tmp = mx.symbol.reshape(s1_tmp, shape=(I1, J1, I2, J2 * Rank))
    s1_tmp = mx.symbol.transpose(s1_tmp, axes=(0, 2, 1, 3))
    s1_tmp = mx.symbol.reshape(s1_tmp, shape=(I1 * I2, J1 * J2 * Rank))
    # multiply InData
    ...
    s2_tmp = mx.symbol.reshape(s2_tmp, shape=(batch_size, I3 * I4, J1 * J2 * Rank))
    s2_tmp = mx.symbol.transpose(s2_tmp, axes=(0, 2, 1))
    s2_tmp = mx.symbol.reshape(s2_tmp, shape=(batch_size * J1 * J2, Rank * I3 * I4))
    # multiply W3 and W4
    s3_tmp = mx.symbol.dot(W3, W4)
    s3_tmp = mx.symbol.reshape(s3_tmp, shape=(Rank * I3, J3, I4, J4))
    s3_tmp = mx.symbol.transpose(s3_tmp, axes=(0, 2, 1, 3))
    s3_tmp = mx.symbol.reshape(s3_tmp, shape=(Rank * I3 * I4, J3 * J4))
...
def resnet(units, num_stage, filter_list, num_class, data_type, bottle_neck=True, bn_mom=0.9, workspace=512,
    ...
    # fc1 = mx.symbol.FullyConnected(data=flat, num_hidden=64 * 4 * 4, name='fc1')
    # fc1 = tucker_fc(flat)
    fc1 = tucker_fc(flat)
