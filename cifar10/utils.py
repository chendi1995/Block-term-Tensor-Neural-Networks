...
def visualize(image, conv_output, conv_grad, gb_viz,No):
    ...
    cam = resize(cam, (24,24), preserve_range=True)

    img = image
    # img = skimage.transform.resize(img, (24, 24))
    # img = img[4:28, 4:28, :]
    # print(img)
    #
    # img -= np.min(img)
    ...
    print(No)
    plt.savefig('./init_img/%d.png'%No,dpi=600)
    ...
    plt.savefig('./output_img/%d.png'%No,dpi=600)
    # plt.show()
