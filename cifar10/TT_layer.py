...
def TT_Layer4(Input, num_layer):
    ...
    if num_layer == 2:
        ...
        d_out = [7, 7, 7, 7]
        TT_r = 2
    W1 = cifar10._variable_with_weight_decay('W1', shape=[d_in[0], TT_r, d_out[0]], stddev=0.4, wd=0.04)
    W2 = cifar10._variable_with_weight_decay('W2', shape=[TT_r, d_in[1], TT_r, d_out[1]], stddev=0.4, wd=0.04)
    W3 = cifar10._variable_with_weight_decay('W3', shape=[TT_r, d_in[2], TT_r, d_out[2]], stddev=0.4, wd=0.04)
    W4 = cifar10._variable_with_weight_decay('W4', shape=[d_in[3], TT_r, d_out[3]], stddev=0.4, wd=0.04)
    ...
    op = 'abc,bdef->adecf'
    W12 = tf.reshape(tf.einsum(op, W1, W2), [d_in[0] * d_in[1], TT_r, d_out[0] * d_out[1]])
    op = 'abcd,ecf->beadf'
    W34 = tf.reshape(tf.einsum(op, W3, W4), [d_in[2] * d_in[3], TT_r, d_out[2] * d_out[3]])
    op = 'abc,dae,ebf->dcf'
    return tf.reshape(tf.einsum(op, W12, Input, W34), [-1, d_out[0] * d_out[1] * d_out[2] * d_out[3]])


def TT_Layer2(Input, num_layer):
    if num_layer == 1:
        d_in = [36, 64]
        d_out = [16, 24]
        Tuk_r = 2
